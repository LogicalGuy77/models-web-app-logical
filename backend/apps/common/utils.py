...
def get_modelmesh_pods(svc, components=[]):
    ...
    For ModelMesh deployments, logs are typically found in the modelmesh-serving pods.
    """
    namespace = svc["metadata"]["namespace"]
...
def get_modelmesh_objects(svc, component):
    ...
    try:
        # In ModelMesh, the actual serving is handled by the ModelMesh controller
        ...
        if "status" in svc and "components" in svc["status"]:
            if component in svc["status"]["components"]:
                ...
                log.info(f"Found ModelMesh predictor status for component {component}")
        ...
        if "spec" in svc and component in svc["spec"]:
            ...
            if runtime_ref:
                ...
                model_format = svc["spec"][component].get("model", {}).get("modelFormat", {}).get("name")
                if model_format:
        ...
        if serving_runtime_name:
            try:
                ...
                log.info(f"Found ServingRuntime {serving_runtime_name} for component {component}")
            except Exception as e:
                log.warning(f"Could not find ServingRuntime {serving_runtime_name}: {e}")
        ...
        try:
            ...
            log.info(f"Found ModelMesh deployment for component {component}")
        except Exception as e:
